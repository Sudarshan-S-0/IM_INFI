...
def send_gmail_notification(config: dict, subject: str, body_text: str, is_html: bool = False) -> bool:
    ...
    smtp_port = gmail_settings.get("smtp_port", 587)
    sender = gmail_settings.get("sender_email")
    password = gmail_settings.get("sender_password")
    receiver = gmail_settings.get("receiver_email")
    
    if not sender or sender == "YOUR_EMAIL@gmail.com" or not password or not receiver:
